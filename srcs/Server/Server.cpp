...
void Server::Run_Server(void)
{
    ...
    while (true)
    {
        ...
        for (pfds_it it = pfds.begin(); it != pfds.end(); it++)
        {
            ...
            if ((it->revents & POLLHUP) == POLLHUP) // event de deconnexion
            {
...
void Server::Interpret_Data(int fd)
{
    std::cout << "i must interpret data" << std::endl;
    try
    {
        (void)fd;
    }
